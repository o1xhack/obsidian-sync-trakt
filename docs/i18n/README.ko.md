...
- **15개 이상 언어를 지원하는 메타데이터** — TMDB를 통해 제목 / 줄거리 / 태그라인 / 장르를 번역합니다. 중국어(간체/번체/홍콩), 일본어, 한국어, 프랑스어, 독일어, 스페인어(스페인/멕시코), 포르투갈어(브라질), 이탈리아어, 러시아어 프리셋과, TMDB가 지원하는 모든 locale을 지정할 수 있는 사용자 정의 모드를 제공합니다. **엄격한 주 언어 + 사용자 지정 폴백**(예: zh-CN의 폴백을 영어로 설정)으로, 주 언어 번역이 없을 때 zh-TW로 조용히 대체되는 일을 방지합니다. 영어 원문은 항상 `*_original_*` frontmatter 필드에 보존됩니다
- **파일명이 언어를 따라감** — 메타데이터 언어를 변경하면 기존 노트가 다음 동기화 때 새 제목에 맞게 자동으로 이름이 바뀝니다. Obsidian 내부 링크도 자동 업데이트됩니다. 설정에는 수동 트리거용 일회성 "지금 이름 바꾸기" 버튼도 있습니다
- **11개 노트 템플릿 언어** — 수작업으로 번역된 노트 템플릿이 en + zh-CN + zh-TW + ja + ko + fr + de + it + es + pt-BR + ru를 커버. 템플릿 언어 드롭다운에서 선택, 언제든지 전환 가능(사용자 정의 내용은 유지)
- **탭 형식 설정 UI** — 일반 / 노트 / 동기화 / Daily Notes 4개 탭. 마지막으로 본 탭은 기기별로 기억
- **Daily Notes 연동** — 동기화할 때마다 각 이벤트(시청 / 시청 목록 추가 / 즐겨찾기 / 평가)를 시간순으로 Daily Note에 자동 삽입. 템플릿 언어를 따름. 마커 구간 내 내용은 플러그인이 관리하며 **구간 외 내용은 절대 수정되지 않음**. **증분 모드**(선택)를 사용하면 마커 구간 내에 직접 작성한 주석도 매 동기화마다 유지됩니다. 수동 백필은 날짜 범위 선택기와 빠른 프리셋(최근 7일 / 이번 달 등)을 사용합니다. Daily Notes는 미디어 노트를 다시 쓰지 않고 독립 자동 동기화 간격으로도 실행할 수 있습니다. [spec 0006](../specs/0006-daily-notes-integration.md) 및 [spec 0011](../specs/0011-daily-notes-auto-sync.md) 참조
- **증분 동기화** — 첫 동기화로 로컬 TMDB 캐시와 Trakt 기록 상태를 채우고, 이후 동기화는 변경된 부분만 가져옵니다. 정상 상태 동기화 시간이 몇 분에서 몇 초로 단축됩니다. [spec 0001](../specs/0001-incremental-sync.md) 참조
- **조용한 쓰기** — 동기화는 **실제로 내용이 변경된** 노트만 다시 씁니다. 새 에피소드 한 편을 시청한 후, 1200개 항목의 라이브러리는 1200개가 아닌 1개의 노트만 다시 쓰여 — Obsidian Sync / iCloud / Syncthing 등 기기 간 동기화 계층이 라이브러리 전체를 매번 다시 업로드하지 않게 됩니다. [spec 0002](../specs/0002-diff-based-write.md) 참조
- **설정별 클라우드 토글** — 각 설정 항목별로 기기 간 동기화 여부를 개별 선택. 예: "Mac에서는 30분마다 자동 동기화, iPhone에서는 완전히 끄기"와 같은 구성 가능. [spec 0003](../specs/0003-device-local-settings.md) 참조
...
- **번들된 노트 템플릿** 11개 언어 — 영어, 간체 중국어(zh-CN), 번체 중국어(zh-TW / zh-HK), 일본어, 한국어, 프랑스어, 독일어, 이탈리아어, 스페인어, 포르투갈어(BR), 러시아어. 기계 번역이 아닌 수작업 번역; 섹션 제목, 항목 라벨, 구두점은 각 언어의 관습을 따름(일본어는 전각 콜론, 프랑스어는 공백 콜론 등). 템플릿 언어 드롭다운은 이 11개만 표시하며, 목록에 없는 locale은 영어로 폴백됩니다(비슷한 방언으로 조용히 대체하지 않음)
...
**안전 계약**: 마커 구간 내 내용은 플러그인이 관리하며, **구간 외 내용은 절대 수정되지 않습니다**. 과거 날짜는 기본적으로 추가만(기존 마커 보존). 오늘 내용은 동기화할 때마다 덮어쓰여 새로운 이벤트가 이후 동기화에서 올바르게 반영됩니다. **증분 모드**를 활성화하면 오늘의 동작도 추가 전용으로 바뀌어, 마커 구간 내에 직접 작성한 주석이 모든 동기화에서 살아남습니다.

**수동 백필**은 날짜 범위 선택기와 빠른 프리셋(최근 7일 / 최근 30일 / 이번 달 / 지난 달)을 사용합니다. 확인하기 전에 선택한 범위 안에 실제로 존재하는 Daily Note의 개수가 실시간으로 표시됩니다. 폴더와 파일명 형식(`YYYY-MM-DD` 또는 `YYYY/YYYY.MM.DD` 같은 Moment.js 구문)은 **설정 → Daily Notes** 에서 구성. [spec 0006](../specs/0006-daily-notes-integration.md) 참조.

**Daily Notes-only 자동 동기화**는 전체 미디어 노트 자동 동기화와 별도로 켤 수 있습니다. Daily Notes에 필요한 Trakt/TMDB 데이터를 새로 고치고 기존 Daily Note 파일만 업데이트합니다. 미디어 노트 생성, 이름 변경, 삭제, 본문 재작성은 하지 않습니다. Daily-only 타이머와 전체 동기화 타이머는 같은 락을 공유하므로 동시에 실행되면 한 실행은 건너뛰고 병렬 쓰기는 발생하지 않습니다.
...
TMDB 메타데이터와 상세 시청 기록 집계처럼 크지만 재구축 가능한 runtime cache는 vault 밖, 각 기기의 로컬 Obsidian 앱 저장소에 저장됩니다. Obsidian Sync로 업로드되지 않으며, 삭제되더라도 각 기기가 Trakt/TMDB에서 다시 구축할 수 있습니다. 작은 동기화된 full-refresh 조정자가 있어, 한 기기가 Trakt 쪽 삭제를 감지한 뒤 다른 기기가 오래된 로컬 캐시에서 상세 기록을 다시 써 버리는 일을 막습니다.

**모든 개별 설정을 기기 간 동기화에서 제외 가능** — 설정 항목 옆의 작은 클라우드 아이콘으로 전환합니다(현재 `Sync on startup` / `Auto-sync` / `Auto-sync interval` / `Daily Notes auto-sync` / `Daily Notes auto-sync interval` / `Plugin UI language`에 노출). 예를 들어 Mac에서는 미디어 노트를 몇 시간마다, Daily Notes를 15분마다 동기화하고 iPhone에서는 모든 자동 타이머를 끄는 구성에 유용합니다.
...
- [x] **0.7** — Daily Notes 연동. 동기화할 때마다 이벤트(시청 / 시청 목록 추가 / 즐겨찾기 / 평가)를 시간순으로 Daily Note에 자동 삽입, 템플릿 언어를 따름. 과거 날짜는 추가만, 오늘 내용은 수시 갱신. → [spec 0006](../specs/0006-daily-notes-integration.md)
- [x] **0.8** — Daily Notes **증분 동기화 모드**. 오늘의 마커 구간을 "전체 교체"에서 "추가 전용"으로 변경하는 옵트인 모드. 이를 통해 구간 내에 직접 작성한 주석이 모든 동기화에서 유지됩니다.
- [x] **0.9** — **메타데이터 언어 폴백**. 「메타데이터 언어」 아래에 "폴백 언어" 드롭다운을 추가. 설정하면 주 언어는 엄격 매칭으로 바뀌고(zh-CN을 찾을 때 zh-TW로 조용히 대체되지 않음), 매칭에 실패하면 사용자가 선택한 폴백을 거쳐, 마지막으로 영어 원문으로 떨어집니다. → [spec 0008](../specs/0008-metadata-language-fallback.md)
- [x] **1.0** — **파일명 자동 변경 + 지속적인 "새 소식" 모달 + 날짜 범위 백필**. 메타데이터 언어를 변경하면 기존 노트가 다음 동기화 때 자동으로 이름이 바뀝니다(Obsidian 내부 링크 자동 업데이트). 새 릴리스마다 일회성 "새 소식" 모달이 표시되어 마지막으로 본 이후의 버전 기록을 보여줍니다. 수동 백필은 날짜 범위 선택기(시작 / 끝 + 빠른 프리셋)로 교체되었습니다. → [spec 0009](../specs/0009-filename-rename.md)
- [x] **1.1** — **vault를 가볍게 유지하는 runtime cache 아키텍처**. 큰 TMDB 캐시와 상세 기록 캐시를 vault 밖으로 옮겨 Obsidian Sync용 `data.json`을 작게 유지하면서 다중 기기 재구축 동작은 보존합니다. → [spec 0010](../specs/0010-local-runtime-cache.md)
- [x] **1.2** — **Daily Notes-only 자동 동기화**. Daily Notes는 자체 간격으로 새로 고칠 수 있으며 미디어 노트를 쓰지 않고, 전체 동기화와 같은 Trakt/TMDB 데이터 경로와 같은 동기화 락을 공유합니다. → [spec 0011](../specs/0011-daily-notes-auto-sync.md)
