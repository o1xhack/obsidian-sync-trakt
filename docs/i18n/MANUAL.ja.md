...
**Bases** タブでは、同期済みメディアノートの既存プロパティを読む
任意の Obsidian データベース形式 `.base` ファイルを作成できます。
Trakt 同期の実行、メディアノートの変更、ネットワーク通信、外部への
データ送信は行いません。

生成される各 Base は、README のライブラリ表示を参考にしたポスター中心
の **Cards** ビューで開きます。縦長ポスター、コンパクトなカード、
小数 1 桁の Trakt 評価、ユーザーが選択したプロパティを表示します。
2 番目の **Details** テーブルも同じ項目を使い、ポスターの
サムネイルを表示します。

| 設定 | デフォルト | 説明 |
|---|---|---|
| Bases folder | `Bases` | 生成した `.base` ファイルの保存先。存在しないフォルダは自動作成されます。 |

生成できるファイル：

| File | Purpose |
|---|---|
| `Movies.base` | 現在の `<prefix>type` プロパティで絞り込んだ映画ノート。 |
| `Shows.base` | 既存の番組ステータスを含む TV 番組ノート。 |
| `Watchlist.base` | 既存の watchlist プロパティが `true` の映画と番組。 |
| `Watched.base` | 既存の watched プロパティが `true` の映画と番組。 |
| `Ratings.base` | 既存の個人評価が `0` より大きい映画と番組。 |
| `Trakt Library.base` | 映画と TV 番組をまとめたオールインワンライブラリ。**Create All-in-one Base** で単独作成でき、**Create All Bases** にも含まれます。 |

**Choose fields shown in each Base** で任意の生成項目を展開すると、
ポスター下と Details テーブルに表示する項目を Base ごとに選べます。
**Recommended** は推奨項目へ戻し、**Select all** は対応する全項目を
表示し、**Clear** は常に表示されるポスターとノートタイトルだけを
残します。

選択肢には、現在プラグインが frontmatter に書き込むすべてのデータが
含まれます。基本メタデータ、タグ、評価、視聴・Watchlist・お気に入り、
再生回数と日時、TV の状態と話数、映画の公開情報、ローカライズ元情報、
ID、外部リンク、同期日時を選択できます。**Episode progress** は
視聴済み話数と放送済み話数から `8 / 10 · 80%` のように計算します。
現在は最後に到達したシーズン・話番号を frontmatter に保存していないため、
その正確な値は Base に表示できません。

既定の並び順は用途別です。映画と番組はコミュニティ評価、Watchlist は
追加日、Watched は最終視聴日時、Ratings は個人評価、ライブラリ全体は
新しい年を優先します。生成後もカードサイズ、表示プロパティ、並び順、
グループ、フィルターを Obsidian で変更できます。

生成される Bases は現在の **Notes folder** と **Property prefix** を
使用します。たとえば、プレフィックスを `trakt_` から `media_` に
変更すると、フィルターと列は `media_type`、`media_title`、
`media_watched` などの既存 `media_*` プロパティを使用します。
いずれかの設定を変更した後は再生成できます。

生成した `.base` ファイルは Obsidian で手動編集できます。対象ファイル
がすでに存在する場合、上書き前に確認が表示されます。上書きを承認すると
その Base の手動編集は置き換えられ、キャンセルすると既存ファイルは
変更されません。Bases ヘルパーはメディアノートを書き換えません。
...
| Community rating/votes update policy | Smart | `trakt_rating` と `trakt_votes` をメディアノートへ書き込む頻度を制御する。`Every sync` は従来の動作を維持し、`Smart` は設定した間隔を超えた場合、または評価/投票数の変化がしきい値に達した場合のみ書き込む。 |
| Minimum update interval | 7 days | _（Smart モードのみ表示）_ 変化が小さくても、この日数を超えるとコミュニティ評価/投票数の書き込みを許可する。 |
| Rating change threshold | `0.1` | _（Smart モードのみ表示）_ Trakt コミュニティ評価の絶対変化量がこの値以上のときに書き込む。 |
| Votes change threshold | `5%` | _（Smart モードのみ表示）_ Trakt 投票数の絶対変化率がこの割合以上のときに書き込む。 |
...
| 設定 | デフォルト | 説明 |
|---|---|---|
| Enable Daily Notes | off | 既存の Daily Note ファイルに、視聴 / watchlist / お気に入り / 評価イベントを書き込めるようにします。存在しない Daily Note ファイルは作成しません。 |
| Daily Notes folder | _(空)_ | Daily Note ファイルがあるフォルダ。空欄の場合は vault ルート。 |
| Daily Notes filename format | `YYYY-MM-DD` | 指定日の Daily Note を探すための Moment.js ファイル名フォーマット。 |
| Start marker / End marker | `%% trakt:daily:start %%` / `%% trakt:daily:end %%` | プラグインはこの 2 つの marker の間だけを管理します。 |
| Auto-update Daily Notes without media-note sync | off | Daily Notes に必要な Trakt/TMDB データを定期的に更新し、既存の Daily Note ファイルだけを更新します。メディアノートの作成、リネーム、削除、再生成は行いません。完全な Auto-sync と同時に有効化でき、2 つのタイマーは同じロックを共有します。デフォルトではデバイスローカル設定です。 |
| Daily Notes auto-sync interval | `60 min` | Daily Notes-only タイマーを実行する頻度。デフォルトではデバイスローカル設定です。 |
| Manual backfill | _(ボタン)_ | 日付範囲ピッカーを開き、既存の過去 Daily Notes に add-only モードで書き込みます。 |
| Today write mode | Default | Default は最新スナップショットで今日の marker 区間を置き換えます。Incremental は新しいイベント行を追記し、marker 内の手書き内容を保持します。 |
