...
| Sync watch history (detailed) | off | 上のトグルの上に重ねて使用。Trakt の `/sync/history` エンドポイントを呼び出し、`{{watch_history}}` テンプレート変数経由でエピソード単位（または映画単位）の視聴タイムスタンプを表示。**アクティブユーザーには大幅に遅い** — 個々の視聴イベントが 1 つの API エントリで、1 ページ 100 件のページネーション。デフォルト OFF；「Sync watch history」が ON のときのみ表示。 |
