...
- Multi-language: **entry verbs (看了 / watched / etc.) follow the
  bundled `templateLanguage` set** (en + zh-CN + zh-TW + 8 more in
  spec 0007). Titles and years inside the entries follow
  `metadataLanguage` as everywhere else in the plugin. UI labels
  follow `uiLanguage`.
...
- **Special cross-device coordination** for `lastDailyNoteSyncedAt`.
  The cursor lives in `data.json` (alongside `lastIncrementalSyncAt`)
  and follows Obsidian Sync like any other shared state. Multi-device
  safety doesn't come from the cursor's storage location — it comes
  from the write rules: today is idempotent (same `historyState` →
  same render), past days are skip-if-markers-present (first device
  fills, others see markers and skip). The cursor is just an
  optimization to avoid scanning all 90 days every sync; even if it's
  wrong on one device, the safety rules prevent corruption
...
**No `dailyNotesEntryTemplate` field in v1.** Entry format is
**hardcoded** as `{time} — {action} {display}`. Settings UI shows a
read-only preview so users know what to expect, but cannot customize
the per-line shape. Keeps v1 focused; customization can be added
later if requested.
...
(Entry format is hardcoded — see note above about no template field.)
...
**No injected warning comment inside the marker block** — in edit
mode users would see it every day, which is noisy. Instead, the
warning lives in the **settings UI** (see "Settings UI" section
below) where users see it during configuration. The marker block
itself stays clean.
...
Following spec 0007's bundled `templateLanguage` set:

| Action | en | zh-CN | zh-TW | ja | ko | fr | de | it | es | pt | ru |
|---|---|---|---|---|---|---|---|---|---|---|---|
| watched | watched | 看了 | 看了 | 視聴 | 시청 | a regardé | hat angeschaut | ha visto | vio | assistiu | посмотрел |
| added_to_watchlist | added to watchlist | 加入想看 | 加入想看 | ウォッチリストに追加 | 시청 목록에 추가 | a ajouté à la liste | zur Watchlist hinzugefügt | ha aggiunto alla watchlist | añadió a la lista | adicionou à lista | добавил в список |
| favorited | favorited | 收藏了 | 收藏了 | お気に入りに追加 | 즐겨찾기 추가 | a mis en favori | als Favorit markiert | ha aggiunto ai preferiti | marcó como favorito | favoritou | добавил в избранное |
| rated | rated `{N}`/10 | 打分 `{N}`/10 | 打分 `{N}`/10 | `{N}`/10 で評価 | `{N}`/10점 평가 | a noté `{N}`/10 | mit `{N}`/10 bewertet | ha valutato `{N}`/10 | calificó `{N}`/10 | avaliou `{N}`/10 | оценил на `{N}`/10 |

`templateLanguage` settings outside this set (e.g. `tr-TR` via custom
mode) fall back to English — same convention as the note body
templates.
...
async function processCatchUp(): Promise<void> {
  ...
  // Always process today FIRST (overwrite mode) — this happens
  // regardless of cursor value, so a same-day repeat sync correctly
  // refreshes today's events. Without this explicit pre-step, a
  // cursor of "today" would cause the while loop below to skip today
  // entirely, leaving newer events un-rendered until tomorrow.
  await processDate(today, { mode: "today" });

  // Then walk PAST days from (last + 1) through (today - 1) in
  // add-only mode. First-ever run (no `last`) starts at today, so
  // the while loop is naturally empty — we don't auto-backfill
  // historically; that's the explicit "Backfill" button's job.
  const SAFETY_CAP = 90;  // see edge case 12
  let cursor = last ? addDays(last, 1) : today;
  ...
  while (cursor < today) {
    await processDate(cursor, { mode: "past" });
...
**No warning comment inside the block.** The "don't edit between
markers" message lives in the settings UI only (see "Settings UI"
section below). Inside the actual Daily Note, the marker block stays
clean.

### One-time backfill (manual button + confirmation modal)

**Settings-only** — the backfill button does NOT register a Command
Palette entry. It's a deliberate one-time-ish action that lives only
in the Daily Notes tab. (The Command Palette gets a different,
narrower command: `Traktr: Sync to Daily Notes (today only)` for
manual today-only re-renders.)

Settings → Daily Notes tab → slider (1-30 days, default 7) + button:
**"Backfill last {N} days"**.

#### Confirmation modal

After click, a modal pops up explaining the safety rules before
...
├─────────────────────────────────────────────────┤
│                                                 │
│ This will check each day from {N} days ago      │
│ through today. For each day:                    │
│                                                 │
│ • If the Daily Note file doesn't exist → skip   │
│                                                 │
│ • If the Daily Note already has our marker      │
│   region → skip (your previous Trakt data       │
│   stays exactly as-is)                          │
│                                                 │
│ • Otherwise → append a fresh marker region with │
│   that day's events to the end of the file      │
│                                                 │
│ Your existing Daily Note content outside our    │
│ marker region is NEVER modified.                │
│                                                 │
│           [Cancel]    [Backfill {N} days]       │
└─────────────────────────────────────────────────┘
```

Same algorithm as catch-up, but starts from `today - N + 1` and
ignores `lastDailyNoteSyncedAt`:
