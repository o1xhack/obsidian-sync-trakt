...
- **Tabbed settings UI** — General / Notes / Bases / Sync / Daily Notes. Last-viewed tab remembered per device
...
- **Quick setup**: open **Settings → Sync Trakt → Bases** to create one all-in-one movie + TV library or separate Movies, Shows, Watchlist, Watched, and Ratings `.base` files. Expand each Base's field chooser to decide exactly what appears below its posters and in its Details table—including watched/watchlist state, favorites, personal and community ratings, play counts, dates, and TV episode progress. Generation is local-only, respects your Notes folder and Property prefix, and never runs a sync or edits media notes.
